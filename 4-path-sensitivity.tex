...
Note that this approach implicitly assumes that a SSA variable always evaluates 
to the same value.
...
needed.
Thus, just relying that \java{b} is the same SSA variable does not guarantee 
that it always evaluates to the same value.
However, without that guarantee any pruning based on \java{b} would introduce 
...
In~\autoref{lst:ps-false-negative}, the SSA variable \java{b} clearly
