...
We have the following properties of these rules:
...
\begin{theorem}[Preservation-ish]
    Let $\bigstep{s}{\store}{\store'}$, $\store$ matches the description $\Gamma$,
    and let $\typestep{\Gamma}{s}{\Delta}$ by the DF rules.
    Then we have that $\store' = \store \dunion \store_0$,
    and $\store_0$ matches the description $\Delta$.
\end{theorem}
%\begin{theorem}
%    TODO do we need this?
%    Let $\typestep{\Gamma}{s}{\Delta}$ be a derivation using the DF rules with $X \in \dom\Delta$.
%    The either $s$ is a command defining $X$ (so it is either an assignment to $X$ or $s$ contains a $\varphi$-node defining $X$),
%    or there exists a subcommand $s_0$ of $s$ with $\typestep{\Gamma_0}{s_0}{\Delta_0}$ and $\Delta_0(X) = \Delta(X)$.
%\end{theorem}
\begin{theorem}[Progress-Ish I]
    Let $\E$ be the derivation tree of $\bigstep{s_s}{\store_s}{\store_s'}$.
    Let $\Gamma_s$ be a description that is matching $\store_s$ and
    $\D_s$ the derivation $\typestep{\Gamma_s}{s_s}{\Delta_s}$.
    Then for all subderivations $\E$ of $\bigstep{s}{\store}{\store'}$ in $\E$
    we have that there exists $\Gamma$ matching $\store$
    in the subderivation $\D$ of $\typestep{\Gamma}{s}{\Delta}$ in $\D_s$.
\end{theorem}
\begin{theorem}[Progress-Ish II]
    Let $\E$ be the derivation tree of $\bigstep{s_s}{\store_s}{\store_s'}$.
    Let $\Gamma_s$ be a description that is matching $\store_s$ and
    $\D_s$ the derivation $\typestep{\Gamma_s}{s_s}{\Delta_s}$.
    Then for all subderivations $\E$ of $\bigstep{a}{\store}{\flagged{}}$ in $\E$
    we have that there exists $\Gamma$ matching $\store$
    in the subderivation $\D$ of $\typestmt{\Gamma}{a}$ in $\D_s$.
\end{theorem}
...
\begin{lemma}
    ...
    Let $\store_s$ match the description $\Gamma_s$
    and $\D_s$ be the derivation of $\typestep{\Gamma_s}{s}{\Delta_s}$.
    If 
    \begin{align*}
        \E = \bsrule{EA-Loc}{}{\bigstep{X}{\store}{\store(X)}}
    \end{align*} 
    with $\store(X) = \tracked{}$ is a part of the derivation tree $\E_s$,
    the corresponding derivation of the read from $X$ in $\D_s$
    \begin{align*}
        \D = \rule{DF-Loc}{}{\typestep{\Gamma}{X}{\Gamma(X)}}
    \end{align*}
    has $\Gamma(X) = \lunknown$ or $\Gamma(X) = \ltracked$.
\end{lemma}
\begin{theorem}
    Let $(s, \Gamma_{ssa}, \Delta_{ssa})$ be a program in SSA form and let $\store_s$ be a store
    with $\dom\store_s = \Gamma_{ssa}$.
...
\begin{proof}
    ...
    By TODO ref Progress-Ish II, we get that $\Gamma$ matches $\store$, so we get 
