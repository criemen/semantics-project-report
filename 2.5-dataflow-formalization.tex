...
% TODO open up arithmetic+boolean definitions
\begin{align*}
    n \in  \:       & \Z\\
...
% add more about gamma and delta up-front, what they mean
...
% TODO change, include semantic rules
The judgement for Booleans and arithmetic expressions does not change.
% sigma hat is not defined before, say more about the notion of abort, not representing the execution-semantics
% stress that this is a pseudo-semantics/tracing semantics, not trying to mirror that the values are tracked at runtime
% perl taint checking
...
% TODO check again, probably wrong
...
% TODO remove join, also adjust text later referencing it
...
\end{align*}
\\
% TODO use square leq instead of leq
Judgement \fbox{$\typestep{\Gamma;\Delta_0;\Delta_1}{\philist}{\Delta}$}:
\begin{align*}
...
% pull out gamma as concretization function, maps lattice points to subsets
% FOLLOWING abstr intr, coceau+coceau reference
...
% in a typing derivation an occurence of a sink is safe, if it is typed with int_c
...
% assume a gamma, then require that the store matches the gamma
% maybe corollary: if something is well-typed with clean gamma and store matches, then no crash
...
% old set gamma(X)=\lclean for all x in \dom\gamma_ssa
\begin{align*}
    \Gamma: &\dom\Gamma_\text{ssa} \to \lattice\\
    &X \mapsto \lclean
\end{align*}
...
% TODO discuss unknown versus tracked
% move it from 3.4 to here
% also, different op-semantics this could be expressed
