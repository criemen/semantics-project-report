...
\begin{definition}[Store-Matching]
    ...
    \begin{align*}
        ...
        L &\mapsto \begin{cases}
            \{\tracked{} | n \in \Z\} & \text{if }L = \ltracked\\
            \{\clean{} | n \in \Z\} & \text{if }L = \lclean\\
            \StoreDomain & \text{if }L = \lunknown\\
        \end{cases}
    \end{align*}
    $\gamma(f) = \{\flagged{} | n \in \Z\}$.
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \bigstepw{\defaultssawhile}{\dom\store}{\store''}{\excstore'}}
        {\bigstep{\defaultssawhile}{\store}{\excstore'}}\\
        &\D = \rule{DF-While}{\typestep{\Gamma;\emptyset;\Delta_1}{\philist}{\Delta}\qquad
        \typestmt{\Gamma \cup \Delta}{b}\qquad\typestep{\Gamma \cup \Delta}{s_0}{\Delta_1}}
        {\typestep{\Gamma}{\defaultssawhile}{\Delta}}
    \end{align*}
    \begin{claim}
        Assume that $\matches{\store_w}{\Gamma \union \Delta}$, that $\dom\Gamma = \filter$ and
        $\bigstepw{\defaultssawhile}{\filter}{\store_w}{\excstore{}'_w}$ by $\E^w$.
        Let $\typestep{\Gamma \union \Delta}{s_0}{\Delta_1}$ by $\D^w_1$
        Then if all sinks in $\E^w$ are safe, $\excstore{}'_w \neq \exception$.

        %.TODO finish
        %Then we have that there exists $\Gamma_w, \Delta_w$ such that 
        %in $\bigstep{s_w}{\store_w}{\excstore_w'}$, $\bigstep{a_w}{\store_w}{\flagged{}}$ and 
        %$\bigstep{b_w}{\store_w}{t}$    
        %with respective derivations 
        %$\typestep{\Gamma_w}{s_w}{\Delta_w}$
    ...
    \begin{claimproof}
        ...
        \end{align*}
        By applying the outer IH on $\E_0$ with $\D^w_1$
        Trivial, $\store_0 = \store_w^1$.\\
        \emph{Subcase 2:}
        \begin{align*}
            \E^w = \bsrule{EW-WhileT}{\overset{}{\bigstep{b}{\store_w}{\btrue}} \enskip
...
\end{theorem}
\begin{proof}
    Now assume that all sinks are safe.
    We prove by induction over the derivation $\E$ that then $\excstore{}' \neq \exception$.
    The cases \textsc{EC-Skip}, \textsc{EC-Sink}, \textsc{EC-Assign}, \textsc{EC-IfT} and \textsc{EC-IfF}
    are trivially clear, as they never evaluate to \exception.\\
    \textbf{Case 1:}
    \begin{align*}
        ...
        {\typestep{\Gamma}{\sink{a}}{\emptyset}}
    \end{align*}
    However, by assumption we have $L = \lclean$.
    Furthermore, as $\store : \Gamma$ by definition, this is a contradiction,
    so this case cannot happen.
    The cases for \textsc{EC-SeqAbrt}, \textsc{EC-IfTAbrt} and \textsc{EC-IfFAbrt}
    ...
    \end{align*}
    \begin{claim}
        Assume that $\matches{\store_w}{\Gamma \union \Delta}$ and that $\dom\Gamma = \Theta$.
        Then we have that there exists $\Gamma_w, \Delta_w$ such that 
        in $\bigstep{s_w}{\store_w}{\excstore_w'}$, $\bigstep{a_w}{\store_w}{\flagged{}}$ and 
        $\bigstep{b_w}{\store_w}{t}$    
        with respective derivations 
...
\iffalse
    ...
    By~\autoref{thm:progress} we get that $\matches{\store}{\Gamma}$, so we get 
