...
\begin{theorem}
    Let $\bigstep{s}{\store}{\store'}$, define $\Gamma: \dom\store \partialf \lattice$ by
    \begin{align*}
        ...
        \end{cases}
    \end{align*}
    and let $\typestep{\Gamma}{s}{\Delta}$.
    Then we have that $\store' = \store \dunion \store_0$,
    $\dom\store_0 = \dom\Delta$
    and that $\Delta$ correctly describes the tracking status of the variables in $\store_0$:
    \begin{align*}
        \forall X: \exists n \in \Z: (\Delta(X) = \lclean \implies \store_0(X) = \clean) 
        \land (\Delta(X) = \ltracked \implies \store_0(X) = \tracked)
    \end{align*}
\end{theorem}

We also want the following (general) soundness property:
\begin{theorem}
    Let $(s, \Gamma_s, \Delta_s)$ be a program in SSA form and let $\store_s$ be a store
    with $\dom\store_s = \Gamma_s$.
    Let $\bigstep{s}{\store_s}{\excstore{}_s}$ be the bigstep derivation.
    Let $\Gamma: \Gamma_s \partialf \lattice$ be defined by
    \begin{align*}
        \Gamma(X) \mapsto \begin{cases}
    ...
    the derivation of $\typestep{\Gamma}{s}{\Delta}$ using the DF ruleset
    contains at least one derivation of the form
    \begin{equation*}
        \rule{DF-Sink}{\typestep{\Gamma}{a}{L}}{\typestep{\Gamma}{\sink{a}}{\emptyset}}
    \end{equation*}
    where $L = \lunknown$ or $L = \ltracked$.
\end{theorem}
\begin{corollary}
