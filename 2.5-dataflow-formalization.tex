...
\begin{definition}[Store-Read Chain]
    ...
    For $i \in \{1, \ldots, m-1\}$ we have that the store described by the derivation
    $\E^1_{i+1}$ reads the variable $X_i$.
    If $\E^1_{i+1}$ uses \textsc{EC-Assign}, we have that 
    $$\E^2_i = \bsrule{EA-Loc}{}{\bigstep{X_i}{\store}{\store(X_i)}}$$
    If $\E^1_{i+1}$ uses case 2, we require that the $\Phi$ node is of form
    $X_{i+1} = \Phi(X_i, Z_{i+1})$.
    In case 3, the $\Phi$ node has to be of form $\Phi(Y_{i+1}, X_i)$.

    A $\Phi$-node is both a read and a store of a variable.
    Thus we require that if $\E^2_i$ is of case 2 or 3 (i.e. a read of a variable in a $\Phi$ node)
    that $\E^2_i = \E^1_{i+1}$.
    This means that a read of the tracked value in a $\Phi$ node is represented as a store in the next element of the 
    store-read chain.
    ...
    TODO $\Phi$ nodes are not addressed here yet
\end{definition}
