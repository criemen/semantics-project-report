...
\end{align*}
We define the set $\tilde{\textbf{Z}} =\{n^f | n \in \textbf{Z}, f \in \{\textbf{c}, \textbf{t}\}\}$.
Stores are denoted with $\sigma \in \Sigma = \textbf{Loc} \dashrightarrow \tilde{\textbf{Z}}$.
Note that stores are partial function from locations to the set of tagged integer
values.
...
\newcommand{\source}[1][a]{\textbf{source }{#1}}
\newcommand{\sink}[1][a]{\textbf{sink }{#1}}
...
\newcommand{\ssaif}[4]{\textbf{if }{#1} \textbf{ then }{#2}\textbf{ else }{#3}}
...
where $\excstore{}: \Loc \partialf \StoreDomain \sqcup \exception$.
...
\begin{align*}
    ...
    {\bigstep{\sink}{\store}{\exception}} \qquad
    \bsrule{EC-Sink}{\bigstep{a}{\store}{\clean{}}}
    {\bigstep{\sink}{\store}{\store}} \qquad
    \bsrule{EC-Skip}{}{\bigstep{\skipcmd}{\store}{\store}}\\
    &\bsrule{EC-SeqExc1}{\bigstep{c_0}{\sigma}{\exception}}
...
\begin{align*}
    \textsc{EA-ToPlainNum}\ddfrac{\langle a, \sigma \rangle \downarrow n^f}
    {\langle a, \sigma \rangle \downarrow n}
\end{align*}
...
\begin{align*}
    c \Coloneqq &\skipcmd \mid \defaultstore \mid \defaultseq \mid (\textbf{if } b \textbf{ then }c_0
    \textbf{ else }c_1);[X = \Phi(Y, Z)] \mid\\
    &\textbf{while }[X = \Phi(Y, Z)]; b\textbf{ do } c_0 \mid \sink \\
\end{align*}
