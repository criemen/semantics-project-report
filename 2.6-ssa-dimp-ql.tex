...
propagate a label if all immediate predecessors have the smae label.
...
and thus does not allow the reconstruction of a typing derivation using the DF rules.

For example, when applied to the example dataflow graph, one can see that there is 
a path from the source to the sink, so that program is unsafe.
