...
% TODO if node is marked with singleton clean -> int_c
% type is a label set
The label $\labclean$ becomes $\lclean$, and the label $\labtracked$ becomes $\ltracked$.
% first describe what's happening, then talk about QL code

% TODO "second substep" 
% After the propagation stabilizes...
...
If a node is assigned only one annotated type (either $\lclean$ or $\lunknown$), 
...
as we construct a solution to the constraints using the join operator.
...
Thus, we assume that the SSA construction already took place.
 %TODO conjecture will be removed
...
% TODO write about how to interpret the analysis results
% a different opsemantics could also make the formal distinction
