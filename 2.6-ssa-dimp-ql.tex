...
% tagged with an annotated type
% make sure annotated type is used consistently
...
%TODO add red empty set to all nodes
\begin{figure}[h]
    \begin{tikzpicture}[roundnode/.style={rectangle,rounded corners=1ex,draw=black!100,thick},->,thick]
...
% label set empty, or label set track, empty set gets mapped to clean, tracked to unknown
% we could still use the same lattice for the theory
% label is now tracked
...
% TODO make this more precise, only for soundness theorem
