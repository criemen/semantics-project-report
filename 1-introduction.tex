...
If for example a compiler can prove that a conditional statement is always false,
...
Besides, open-source projects hosted on GitHub and other open-source platforms could
...
which allows the analysis of any software, not only publicly hosted one.
...
We expect that these links are stable, i.e.\ the content does not change over time.
...
project fits in. 
The CodeQL tool is built upon the domain specific language QL that enables declarative,
...
For every supported language by the CodeQL analyzers, developers usually re-implement all of these libraries from
...
so in the theoretical setting we are dealing with a (provably) sound analysis.
